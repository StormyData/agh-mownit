...
def text_phantom(text, size, imsize, angle=None) -> np.ndarray:
    font = '/usr/share/fonts/noto/NotoSerifDisplay-Thin.ttf'
...
precedeed['b'] = set('hl')
precedeed['c'] = set('moe')
precedeed['e'] = set('hijmn')
precedeed['h'] = set('lm')
precedeed['i'] = set("bfhklp14")
precedeed['j'] = set("bdfhklp14")
precedeed['l'] = set("bdhkp")
precedeed['m'] = set("hijkln")
precedeed['n'] = set("hijlmu")
precedeed['o'] = set("bdp0")
precedeed['t'] = set("bdfhjklmpr014")
precedeed['u'] = set("hijlmn")
precedeed['1'] = set("4")
precedeed['3'] = set("8")
precedeed['6'] = set("0")
precedeed['.'] = set("abcdefhijklmnopru01234569,?!")
precedeed[','] = set("4")
precedeed["!"] = set("bdfhjkp14")
...
def generate_pair_width(pair, patterns, space_width):
    ...
    if char_1 not in boxes or char_2 not in boxes or (
        ...
        left_char = boxes[char_1][0]
        right_char = boxes[char_2][0] if char_1 != char_2 else boxes[char_1][1]
        left_end = left_char[1]
        right_begin = right_char[1] - right_char[3]
...
def main(img: np.ndarray):
    ...
    for key, val in line_separated_boxes.items():
        ...
        tmp = val
        #tmp = list(
         #   filter(lambda x: x[0] not in ".,lr!i", val))  # filtering out characters, that are not properly recognized
    ...
    for char in patterns:
        ...
        for box in boxes:
            for i in range(len(combined)):
                if get_box_overlap(box, combined[i]) > 0.1:
...
if __name__ == "__main__":
    txt = "\n".join(["".join([c for c in "abcdefghijklmnoprstuwxyz0123456789.,?!"])] * 2)
    # txt = # "lorem ipsum 123 !!! ?"
    img = text_phantom(txt, 100, (2000, 250))
    draw(img)
